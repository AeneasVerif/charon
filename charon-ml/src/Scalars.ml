...
let get_val (integer : integer_value) =
  match integer with
  | SignedInteger (_, v) | UnsignedInteger (_, v) -> v

let get_ty (integer : integer_value) =
  match integer with
  | SignedInteger (int_ty, _) -> Signed int_ty
  | UnsignedInteger (uint_ty, _) -> Unsigned uint_ty

(** Check that an integer value is in range *)
let check_integer_value_in_range ptr_size (v : integer_value) : bool =
  check_int_in_range ptr_size (get_ty v) (get_val v)

(** Make an integer value, while checking the value is in range *)
let mk_integer_value ptr_size (int_ty : integer_type) (i : big_int) :
    (integer_value, unit) result =
  if check_int_in_range ptr_size int_ty i then
    Ok
      (match int_ty with
      | Signed int_ty -> SignedInteger (int_ty, i)
      | Unsigned uint_ty -> UnsignedInteger (uint_ty, i))
  else Error ()
