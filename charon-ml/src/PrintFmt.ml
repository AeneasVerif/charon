...
(** Whether Rust treats the declaration this refers to specially. We only use
    this to print such types the way they're spelled in Rust. *)
and type_decl_ref_builtin (env : fmt_env) (tref : type_decl_ref) :
    builtin_ty option =
  match TypeDeclId.Map.find_opt tref.id env.crate.type_decls with
  | Some { src = BuiltinType builtin; _ } -> Some builtin
  | _ -> None

(** Whether this refers to a declaration that monomorphization specialized. *)
and type_decl_ref_is_specialized (env : fmt_env) (tref : type_decl_ref) : bool =
  match TypeDeclId.Map.find_opt tref.id env.crate.type_decls with
  | Some decl -> type_decl_is_specialized decl
  | None -> false
...
and pp_const_aggregate (env : fmt_env) (tref : type_decl_ref)
    (builtin : builtin_ty option) opt_variant_id (fmt : Format.formatter)
    (fields : constant_expr list) : unit =
  match builtin with
  | Some TTuple ->
...
and pp_constant_expr (env : fmt_env) (fmt : Format.formatter)
  ...
  | CAdt (variant_id, fields) -> begin
      match cv.ty with
      | TAdt tref ->
          pp_const_aggregate env tref tref.builtin variant_id fmt fields
...
and pp_ty (env : fmt_env) (fmt : Format.formatter) (ty : ty) : unit =
  ...
  | TAdt ({ builtin = Some TTuple; _ } as tref) -> (
      ...
      match Substitute.ty_as_tuple_fields env.crate.type_decls ty with
      | Some fields ->
          let trailing_comma = if List.length fields = 1 then "," else "" in
          Format.fprintf fmt "(%a%s)"
            (pp_sep_list ", " (pp_ty env))
            fields trailing_comma
      | None -> pp_type_decl_ref env fmt tref)
...
let rec pp_projection_elem (env : fmt_env) (subplace : place)
  ...
  | Field (opt_variant_id, fid) -> (
      (* The type of the sub-place tells us which declaration the field comes
         from. *)
      let adt_id = Option.map fst (ty_as_opt_adt subplace.ty) in
      let field_name =
        match
          Option.bind adt_id (fun adt_id ->
              adt_field_to_string env adt_id opt_variant_id fid)
        with
        | Some field_name -> field_name
        | None -> FieldId.to_string fid
      in
      match (opt_variant_id, adt_id) with
      | None, _ -> Format.fprintf fmt "%s.%s" sub field_name
      | Some variant_id, Some adt_id ->
...
and pp_aggregate (env : fmt_env) (agg : aggregate_kind) (fmt : Format.formatter)
  ...
  | AggregatedAdt (tref, opt_variant_id, opt_field_id) -> (
      match type_decl_ref_builtin env tref with
      | Some TTuple ->
          let trailing_comma = if List.length fields = 1 then "," else "" in
          Format.fprintf fmt "(%a%s)"
            (pp_sep_list ", " pp_string)
