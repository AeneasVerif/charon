...
(** Whether this declaration is one that monomorphization made for a single
    instantiation of another. *)
let type_decl_is_specialized (def : type_decl) : bool =
  List.exists
    (function
      | PeInstantiated _ -> true
      | _ -> false)
    def.item_meta.name
...
let ty_as_opt_adt (ty : ty) : (type_decl_id * generic_args) option =
  match ty with
...
let ty_as_adt (ty : ty) : type_decl_id * generic_args =
