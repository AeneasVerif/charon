...
(** The id of a translated item. *) and  any_decl_id = 
...
(** Reference to a function declaration. *) type  fun_decl_ref = { fun_id :  fun_decl_id  ;fun_generics : ( generic_args)  (** Generic arguments passed to the function. *) }

(** Reference to a global declaration. *) and  global_decl_ref = { global_id :  global_decl_id  ;global_generics : ( generic_args)   }
 ...
 *) and  trait_instance_id = 
 ...
 | TraitImpl of  trait_impl_id *( generic_args)  (** A specific top-level implementation item. *)
 ...
 | ParentClause of ( trait_instance_id) * trait_decl_id * trait_clause_id  (** A parent clause
 ...
 | BuiltinOrAuto of ( trait_decl_ref) region_binder *( trait_ref) list *(( trait_item_name)*( ty)) list  (** A trait implementation that is computed by the compiler, such as for built-in trait
 ...
 | Dyn of ( trait_decl_ref) region_binder  (** The automatically-generated implementation for [dyn Trait]. *)
 ...
 *) and  trait_decl_ref = { trait_decl_id :  trait_decl_id  ;decl_generics : ( generic_args)   }

(** A reference to a tait impl, using the provided arguments. *) and  trait_impl_ref = { trait_impl_id :  trait_impl_id  ;impl_generics : ( generic_args)   }

(** Each [GenericArgs] is meant for a corresponding [GenericParams]; this describes which one. *) and  generics_source = 
 ...
 *) and  type_id = 
 ...
 | TAdt of  type_id * generic_args  (** An ADT.
 ...
 | TDynTrait of  existential_predicate  (** [dyn Trait]
 ...
 | TArrow of ((( ty) list)*( ty)) region_binder  (** Arrow type, used for function pointers and reused for the unique type associated with each
          function item.
 ...
 *) and 'a0 binder = { binder_params :  generic_params  ;binder_value : 'a0  (** Named this way to highlight accesses to the inner value that might be handling parameters
        incorrectly. Prefer using helper methods.
     *) }
 ...
 *) and  variant_layout = { field_offsets : int list  (** The offset of each field. [None] if it is not knowable at this point, either because of
 ...
 *) and  layout = { size : int option  (** The size of the type in bytes. *);align : int option  (** The alignment, in bytes. *);discriminant_offset : int option  (** The discriminant's offset, if any. Only relevant for types with multiple variants. *);uninhabited : bool  (** Whether the type is uninhabited, i.e. has any valid value at all.
...
(** A type declaration.
    ...
    inlined in MIR.
 *) and  type_decl = { def_id :  type_decl_id  ;item_meta :  item_meta  (** Meta information associated with the item. *);generics :  generic_params  ;kind :  type_decl_kind  (** The type kind: enum, struct, or opaque. *);layout : ( layout) option  (** The layout of the type. Information may be partial because of generics or dynamically-
