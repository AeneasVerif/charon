...
(** A variable *) type  local = { index :  local_id  (** Unique index identifying the variable *);name : ( string) option  (** Variable name - may be [None] if the variable was introduced by Rust
        through desugaring.
 ...
 *) and  assertion = { cond :  operand  ;expected : bool  (** The value that the operand should evaluate to for the assert to succeed. *);on_failure :  abort_kind  (** What kind of abort happens on assert failure. *) }
...
(** Additional information for closures. *) and  closure_info = { kind :  closure_kind  ;signature : ((( ty) list)*( ty)) region_binder  (** The signature of the function that this closure represents. *) }
 ...
 *) type 'a0 gexpr_body = { span :  span  ;locals :  locals  (** The local variables. *);body : 'a0   }
 ...
 and  cli_options = { ullbc : bool  (** Extract the unstructured LLBC (i.e., don't reconstruct the control-flow) *);lib : bool  (** Compile the package's library *);bin : ( string) option  (** Compile the specified binary *);mir_promoted : bool  (** Deprecated: use [--mir promoted] instead. *);mir_optimized : bool  (** Deprecated: use [--mir optimized] instead. *);mir : ( mir_level) option  (** The MIR stage to extract. This is only relevant for the current crate; for dpendencies only
     ...
     *);skip_borrowck : bool  (** If activated, this skips borrow-checking of the crate. *);monomorphize : bool  (** Monomorphize the code, replacing generics with their concrete types. *);extract_opaque_bodies : bool  (** Usually we skip the bodies of foreign methods and structs with private fields. When this
        flag is on, we don't.
     ...
     *);included : ( string) list  (** Whitelist of items to translate. These use the name-matcher syntax. *);opaque : ( string) list  (** Blacklist of items to keep opaque. These use the name-matcher syntax. *);exclude : ( string) list  (** Blacklist of items to not translate at all. These use the name-matcher syntax. *);remove_associated_types : ( string) list  (** List of traits for which we transform associated types to type parameters. *);hide_marker_traits : bool  (** Whether to hide the [Sized], [Sync], [Send] and [Unpin] marker traits anywhere they show
        up.
     *);remove_unused_self_clauses : bool  (** Trait method declarations take a [Self: Trait] clause as parameter, so that they can be
