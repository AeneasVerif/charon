...
impl ItemTransCtx<'_, '_> {
    ...
    fn adt_self_ty(&mut self, span: Span, def_id: &hax::DefId) -> Result<Ty, Error> {
        ...
        let self_ty = TyKind::Adt(
            TypeId::Adt(self_decl_id),
            self.the_only_binder()
                .params
                .identity_args(GenericsSource::item(self_decl_id)),
        )
        .into_ty();
        Ok(self_ty)
    }
    ...
    fn adt_drop_predicate(
        ...
        let implemented_trait = TraitDeclRef {
            ...
            generics: Box::new(GenericArgs::new_types(
                [self_ty.clone()].into(),
                GenericsSource::item(drop_trait),
            )),
        };
    ...
    pub fn translate_drop_method(
        ...
        let impl_ref = TraitImplRef {
            impl_id: drop_impl_id,
            generics: Box::new(
                self.the_only_binder()
                    .params
                    .identity_args(GenericsSource::item(drop_impl_id)),
            ),
        };
    ...
    pub fn translate_drop_impl(
        ...
        let implemented_trait = self.adt_drop_predicate(span, def.def_id())?;
        let drop_trait = implemented_trait.trait_id;
        ...
        let method_binder = {
            ...
            let generics = self
                .outermost_binder()
                .params
                .identity_args_at_depth(GenericsSource::item(impl_id), DeBruijnId::one())
                .concat(
                    GenericsSource::item(method_id),
                    &method_params.identity_args_at_depth(
                        GenericsSource::Method(drop_trait, method_name.clone()),
                        DeBruijnId::zero(),
                    ),
                );
            Binder::new(
