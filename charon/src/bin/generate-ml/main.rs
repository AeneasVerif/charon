...
impl<'a> GenerateCtx<'a> {
    ...
    /// parameters.
    fn type_to_ocaml_name(&self, ty: &Ty) -> String {
        match ty.kind() {
                    ...
                    TypeId::Adt(id) => {
                        let mut base_ty = if let Some(tdecl) = self.crate_data.type_decls.get(id) {
                            ...
                            eprintln!(
                                ...
                                repr_name(self.crate_data, name)
                            );
                            name.name
                                .last()
                                .unwrap()
    ...
    fn type_decl_to_json_deserializer(&self, decl: &TypeDecl) -> String {
        ...
        for (i, (kind, names)) in self.markers.iter().enumerate() {
            ...
            let generated = match kind {
                GenerationKind::OfJson => {
                    let fns = tys
                        .map(|ty| ctx.type_decl_to_json_deserializer(ty))
                        .format("\n");
                    format!("let rec ___ = ()\n{fns}")
                }
                GenerationKind::TypeDecl(visitors) => {
                    let mut decls = tys
                        ...
                        .map(|(i, ty)| {
                            ...
                            ctx.type_decl_to_ocaml_decl(ty, co_recursive)
                        })
                        .join("\n");
                    if let Some(visitors) = visitors {
                        ...
                        if !extra_types.is_empty() {
                            ...
                            let intermediate_visitor = ctx.generate_visitor_bases(
