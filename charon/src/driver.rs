...
/// The callbacks for the regular rustc compiler.
/// We need it because we call Charon by using cargo (and changing the value
/// of the environment variable RUSTC_WORKSPACE_WRAPPER) which means Charon
/// gets also called to compile the dependencies: when this happens, we fall
/// back to calling rustc.
...
fn main() {
    ...
    // It seems the charon-driver is only called on the target (not the
    // dependencies) which means we don't have to check if we need to
    // call the default Rust callbacks or not.
    // Call the Rust compiler with the proper call-back.
    RunCompiler::new(
        &compiler_args,
        &mut CharonCallbacks {
    ...
    /*    RunCompiler::new(&compiler_args, &mut RustcCallbacks {})
            .run()
            .unwrap();
    */
    ...
    let (_manifest, package, mut external_deps) =
        read_manifest_compute_external_deps(&options, &options.input_file);

    // Call the Rust compiler with the proper options
    let mut compiler_args = vec![
        exec_path,
        sysroot_arg,
        options.input_file.as_path().to_str().unwrap().to_string(),
