            ...
            let meta = match &type_decl.ptr_metadata {
                PtrMetadata::InheritFrom(ty) => {
                    match ty.kind() {
                        TyKind::TypeVar(..) => {
                            ...
                            if is_sized {
                                trace!(
                                    ...
                                    type_decl.def_id.with_ctx(fmt)
                                );
                                PtrMetadata::None
                            } else {
                                trace!(
                                    "Ptr-metadata for type {} inheriting from {} is not Sized, keep inheritance.",
                                    type_decl.def_id.with_ctx(fmt),
                                    ty.with_ctx(fmt)
                                );
                        ...
                        _ => {
                            trace!(
                                "Non-type-var inheritance found for type: {}, inheriting from: {}",
                                type_decl.def_id.with_ctx(fmt),
                                ty.with_ctx(fmt)
                            );
                            PtrMetadata::InheritFrom(ty.clone())
