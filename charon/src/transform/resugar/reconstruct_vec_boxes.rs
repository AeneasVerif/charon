...
const LANG_ITEM_BOX_ASSUME_INIT_INTO_VEC_UNSAFE: &str = "box_assume_init_into_vec_unsafe";
...
impl FusedUllbcPass for Transform {
    ...
    fn transform_body(&self, ctx: &mut TransformCtx, body: &mut ExprBody) {
        ...
        let rewrites = body.body.iter_enumerated().filter_map(|(drop_bid, block)| {
            // drop (dropped_box)
            let (_kind, dropped_box, _trait_ref, target, on_unwind) =
                block.terminator.kind.as_drop()?;
