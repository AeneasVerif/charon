...
use crate::{
    ast::{TypeDeclId, layout_guarantees::LayoutGuarantees},
    ...
    fn transform_ctx(&self, ctx: &mut TransformCtx) {
        ...
        for type_decl in ctx.translated.type_decls.iter() {
            if let Some(aliased_ty) = type_decl.kind.as_alias() {
                aliases.push((type_decl.def_id, aliased_ty.clone()));
            }
        }

        let alias_guarantees: Vec<(TypeDeclId, LayoutGuarantees)> = aliases
            .into_iter()
            .filter_map(|(id, ty)| {
                if let Some(guarantees) = LayoutGuarantees::for_ty(&ty, &ctx.translated) {
                    Some((id, guarantees))
                } else {
                    None
                }
        ...
        for (td_id, guarantee) in alias_guarantees.into_iter() {
            let type_decl = type_decls.get_mut(td_id).unwrap();
            for (_, layout) in type_decl.layout.iter_mut() {
                layout.update_guarantees(guarantee.clone());
            }
        }
    }
}
