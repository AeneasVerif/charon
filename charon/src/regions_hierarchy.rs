...
type LifetimeConstraints = DiGraphMap<Region, ()>;
...
fn compute_sccs_from_lifetime_constraints(
    ...
    // Reorder the SCCs
    let get_region_parents = &|r: Region| {
        constraints_graph
    ...
    let all_rids: Vec<Region> = all_var_regions
...
fn compute_regions_hierarchy_from_constraints(mut constraints: SCCs<Region>) -> RegionGroups {
...
pub type RegionVarsConstraintsMap = LinkedHashMap<RegionId::Id, HashSet<Region>>;

/// See [TypesConstraintsMap]
pub type TypeVarsConstraintsMap = LinkedHashMap<TypeVarId::Id, HashSet<Region>>;
...
fn add_region_constraints(
    ...
    region: Region,
    parent_regions: &im::HashSet<Region>,
) {
...
fn compute_full_regions_constraints_for_ty(
    ...
    match ty {
        Ty::Adt(type_id, generics) => {
            ...
            let region_inst: RegionId::Vector<Region> =
...
fn compute_regions_constraints_for_type_decl_group(
    ...
    // Compute the SCCs
    let mut sccs_vec: Vec<SCCs<Region>> = Vec::new();
    for id in type_ids.iter() {
        let type_def = types.get(*id).unwrap();
...
where
    T: Formatter<TypeVarId::Id> + Formatter<RegionId::Id> + Formatter<&'a Region>,
{
