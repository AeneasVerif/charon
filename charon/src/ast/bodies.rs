...
pub enum Body {
    ...
    Extern(String),
    /// Rust intrinsic function. This has no body and describes a "built-in" operation that must be
    /// handled by the codegen backend.
    Intrinsic {
        /// The intrinsic name.
        name: String,
...
pub struct Call {
    ...
    pub dest: Place,
    /// Whether the compiler guarantees this call is safe even though the callee's signature is
    /// unsafe. Only set in that case: for desugared drops (drop glue is unsafe) and for calls to
    /// safe `#[target_feature]` functions from contexts that enable the required features.
...
/// The macro that generated this inline assembly.
#[derive(Debug, Copy, Clone, PartialEq, Eq)]
#[derive(EnumIsA)]
#[derive(SerializeState, DeserializeState, Drive, DriveMut, DriveTwo)]
pub enum AsmKind {
    /// Assembly generated by the `asm!` macro.
    Asm,
    /// Assembly generated by the `naked_asm!` macro.
    NakedAsm,
}
