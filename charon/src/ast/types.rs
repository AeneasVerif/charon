...
pub enum TyKind {
    ...
    /// There's a binder here because charon function items bind both early and late-bound variables;
    /// that binder binds the late-bound lifetimes (those that could appear in a function pointer type
    /// like `for<'a> fn(&'a u32)`).
