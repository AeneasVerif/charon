...
fn test_compare_to_map() {
    let mut tree = AvlTree::<i32, i32>::new();
    let mut bmap = std::collections::BTreeMap::<i32, i32>::new();

    let _ = Vec::from([6, 1, 0, 4, 4, 3, 9, 12, 13, 4, 2, 1, 30, 40, 2, 7])
        .into_iter()
        .map(|n| {
            assert_eq!(
                tree.insert(n, n * 10),
    ...
    let _ = Vec::from([6, 1, 0, 7, 3, 9, 12, 13, 14, 1, 30, 40, 2, 7])
        .into_iter()
        .map(|n| {
            assert_eq!(tree.get(&n), bmap.get(&n), "While searching {}", n);
            tree.check_integrity();
            if !bmap.contains_key(&n) {
                return;
            };

            let t = tree.get_mut(&n).unwrap();
            let m = bmap.get_mut(&n).unwrap();
            *t += 1;
            *m += 1;
            assert_eq!(tree.get(&n), bmap.get(&n), "While searching {}", n);
            tree.check_integrity();
    ...
    let _ = Vec::from([7, 30, 20, 13, 12, 9, 5, 0, 1, 2])
        .into_iter()
        .map(|n| {
            assert_eq!(tree.remove(&n), bmap.remove(&n), "While removing {}", n);
            tree.check_integrity();
        });

    let _ = Vec::from([30, 40, 50, 60, 45, 35, 25])
        .into_iter()
        .map(|n| {
            assert_eq!(
                tree.insert(n, n * 10),
    ...
    let t_values = tree
        .into_iter()
        .collect::<std::vec::Vec<(&i32, &mut i32)>>();
    let m_values = (&mut bmap)
        .into_iter()
        .collect::<std::vec::Vec<(&i32, &mut i32)>>();
