...
fn test_compare_to_map() {
    ...
    [6, 1, 0, 4, 4, 3, 9, 12, 13, 4, 2, 1, 30, 40, 2, 7].map(|n| {
        assert_eq!(tree.insert(n, n * 10), bmap.insert(n, n * 10), "While inserting {}", n);
        tree.check_integrity();
    });
    ...
    let m_values = (&mut bmap).into_iter().collect::<std::vec::Vec<(&i32, &mut i32)>>();
